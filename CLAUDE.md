...
### Release Workflow
- `permissions: contents: write` required at job/workflow level for `softprops/action-gh-release` — GitHub default token only has read, causes 403 without it
- Artifact paths in `action-gh-release`: use glob `/**` suffix (e.g. `Runner.app/**`) not bare paths
- Release job may exit non-zero but release IS created — check GitHub release directly, not just exit code
- Re-run failed release workflow: `git tag -d <tag> && git push origin :refs/tags/<tag> && git tag <tag> && git push origin <tag>`
- Publishing draft releases: `gh release edit <tag> --repo <repo> --draft=false --latest`

### Flutter Linux Build Dependencies
- `libgtk-3-dev libsecret-1-dev keybinder-3.0 libayatana-appindicator3-dev` — required by `flutter_secure_storage_linux`, `hotkey_manager_linux`, `tray_manager_linux`

### Matrix Step Guards
- Use `if: matrix.os == 'ubuntu-latest'` on Linux-specific steps (e.g. `apt-get install`) to prevent them from running on Windows/macOS runners

### /loop Usage
- Use `/loop <interval>` when monitoring CI runs or async operations (e.g. `/loop 2m check deploy`)
- When triggering tag-based release: run `/loop` immediately, use ~90-120s intervals (builds take 3-8 min)
- Stop when task completes, fails, or user interrupts
